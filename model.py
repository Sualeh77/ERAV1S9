...
########################################################################  S8 | Cifar 10   #########################################################################
class Cifar10_S8(nn.Module):
    def __init__(self, device, norm:str="bn"):
        super(Cifar10_S8, self).__init__()
        self.device = device
        self.norm = norm
...
class Cifar10_S9(nn.Module):
    def __init__(self, device, norm:str="bn"):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3, padding=1),  # 3x32x32 > 32xx32x32 > RF 3
            nn.ReLU(),
            nn.GroupNorm(4, 32) if self.norm=="gn" else nn.GroupNorm(1, 32) if self.norm=="ln" else nn.BatchNorm2d(32),
            nn.Dropout(0.05),
            nn.Conv2d(32, 64, 3, padding=1),    # 16x32x32 > 64x32x32 | RF 5
            nn.ReLU(),
            nn.GroupNorm(4, 64) if self.norm=="gn" else nn.GroupNorm(1, 64) if self.norm=="ln" else nn.BatchNorm2d(64),
            nn.Dropout(0.05),
            #nn.Conv2d(64, 32, 3, stride=2)  # 64x32x32 > 32x15x15 | RF 9 | J 2
            nn.Conv2d(64, 32, 3, stride=2, dilation=2, padding=2)  # 64x32x32 > 32x32x16 | RF 9 | J 2  | Dilated Conv instead of Maxpool
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(32, 32, 3, padding=1),    # 32x16x16 > 32x16x16 | RF 13
            nn.ReLU(),
            nn.GroupNorm(4, 32) if self.norm=="gn" else nn.GroupNorm(1, 32) if self.norm=="ln" else nn.BatchNorm2d(32),
            nn.Dropout(0.05),
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, dilation=2, padding=2),   # 32x16x16 > 64x16x16 | RF 21 | Dilated Convolution**
            nn.ReLU(),
            nn.GroupNorm(4, 64) if self.norm=="gn" else nn.GroupNorm(1, 64) if self.norm=="ln" else nn.BatchNorm2d(64),
            nn.Dropout(0.05),
            nn.Conv2d(64, 32, 3, stride=2)    # 64x15x15 > 32x7x7 | RF 29 | J 4
            #nn.Conv2d(64, 32, 3, stride=2, dilation=2, padding=2)    # 64x16x16 > 32x8x8 | RF 29 | J 4
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(32, 32, 3, padding=1),    # 32x8x8 > 32x8x8 | RF 37
            nn.ReLU(),
            nn.GroupNorm(4, 32) if self.norm=="gn" else nn.GroupNorm(1, 32) if self.norm=="ln" else nn.BatchNorm2d(32),
            nn.Dropout(0.05),
            nn.Conv2d(32, 32, 3, padding=1, groups=32),  # 32x8x8 > 32x8x8 | RF 45  | Depthwise Conv: Layer(=i/p ch.) implementation
            nn.Conv2d(32, 64, kernel_size=1),   # 32x8x8 > 64x8x8 | RF 45   | Depthwise Conv: Point Conv to complete Depthwise implementation
            nn.ReLU(),
            nn.GroupNorm(4, 64) if self.norm=="gn" else nn.GroupNorm(1, 64) if self.norm=="ln" else nn.BatchNorm2d(64),
            nn.Dropout(0.05),
            nn.Conv2d(64, 32, 3, stride=2)    # 64x7x7 > 32x3x3 | RF 61 | J 8
            #nn.Conv2d(64, 32, 3, stride=2, dilation=2, padding=2)    # 64x8x8 > 32x4x4 | RF 61 | J 8
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(32, 32, 3, padding=1),  # 32x4x4 > 64x4x4 | RF 77
            nn.ReLU(),
            nn.GroupNorm(4, 32) if self.norm=="gn" else nn.GroupNorm(1, 32) if self.norm=="ln" else nn.BatchNorm2d(32),
            nn.Dropout(0.05),
            nn.Conv2d(32, 64, 3, padding=1),  # 64x4x4 > 64x4x4 | RF 93
            nn.ReLU(),
            nn.GroupNorm(4, 64) if self.norm=="gn" else nn.GroupNorm(1, 64) if self.norm=="ln" else nn.BatchNorm2d(64),
            nn.Dropout(0.05),
            nn.Conv2d(64, 64, 3, padding=1)   # 64x4x4 > 64x4x4 | RF 109
        )
        self.gap = nn.AdaptiveAvgPool2d((1, 1))     # 64x4x4 > 64x1x1
        self.output = nn.Conv2d(64, 10, 1)     # 64x1x1 > 10x1x1
